...
if __name__ == "__main__":
  ...
  for controller_cat, controller_type in [('baseline', args.baseline_controller), ('test', args.test_controller)]:
    ...
    results = []
    for f in tqdm(files[SAMPLE_ROLLOUTS:]):
      results.append(rollout_partial(f))
    # results = process_map(rollout_partial, files[SAMPLE_ROLLOUTS:], max_workers=1, chunksize=1)
