from tinyphysics import TinyPhysicsModel, TinyPhysicsSimulator, CONTEXT_LENGTH, MAX_ACC_DELTA, CONTROL_START_IDX, STEER_RANGE, State, FuturePlan
...
class TinyPhysicsEnv(Env, TinyPhysicsSimulator):
    def __init__(self, model_path='models/tinyphysics.onnx', data_path = 'data/00000.csv'):
        ...
        self.observation_space = Box(-20, 20, (3,))
    ...
    def step(self, action):
        ...
        if self.step_idx == len(self.target_lataccel_history):
            ...
            obs = np.array([self.target_lataccel_history[self.step_idx], self.current_lataccel, self.state_history[self.step_idx], self.futureplan]) 
        reward = np.sum(list(self.compute_cost().values()))
